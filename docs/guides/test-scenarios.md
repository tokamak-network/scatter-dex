...
Legend: [x] = implemented in scenarios.test.ts, [ ] = requires integration/on-chain test

## User Scenarios

### [x] U1: Basic Order Lifecycle (Happy Path)
...
### [x] U2: Cross-Relayer Settlement (Happy Path)
...
2. User A claims via Relayer A gasless API → works (Relayer A settled the claims root)
3. User B claims via Relayer B gasless API → works IF Relayer B also recorded the claims root
4. Edge: User B tries gasless claim from Relayer A → rejected (Relayer A only pays gas for its own settlements)
5. Fallback: User B can always claim directly on-chain via wallet (no relayer needed)
...
### [x] U8: Wrong Relayer in Signature
...
### [x] R5: Shared Orderbook Server Down
...
### [x] R7: Reactive Matching (WS Notification)
...
### [x] E7: Same Nonce Reuse After Cancellation
...
### [x] E8: Multiple Relayers Same User
