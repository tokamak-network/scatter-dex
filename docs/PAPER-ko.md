...
Tornado Cash는 법 집행기관에 협력할 수 있는 책임 있는 중개자가 없었기 때문에 OFAC의 제재를 받았다. Renegade는 비용이 많이 드는 다자간 계산을 통해 프라이버시를 달성한다. Railgun은 ZK 프라이버시를 제공하지만 규정 준수 메커니즘이 없다. Jigsaw [23]는 최근에 제안된 프라이버시-거래 원시(primitive)로 본 발명과 가장 가까운 선행기술이지만, 책임성 및 규제기관 감사(audit) 층위는 설계 범위 밖에 두고 있다. 기존 시스템 중 세 가지를 모두 해결하는 것은 없다.
...
Jigsaw (Cryptology ePrint 2025/1147)는 프라이버시-거래 설계 공간에서 본 발명에 가장 가까이 위치한 공개 구성으로, 본 출원의 선행기술 조사에서 **가장 가까운 선행기술(closest prior art)** 로 식별되었다. Jigsaw는 DeFi용 ZK 기반 프라이버시 거래 원시(primitive)를 제안하며, 정산 수령자와 분리된 은닉 잔고 위에서 작동하고, nullifier 기반 이중지출 방지에 의존한다는 점에서 zkScatter의 Layer 1/2 분리와 겹치는 요소가 있다.

zkScatter가 Jigsaw와 구별되는 지점은 다음과 같으며, 각각은 본 발명의 본질적 구성요소이다:

1. **거래-클레임 분리 (Layer 3).** Jigsaw는 정산 시점에 수령자가 노출된다 — 거래가 정산될 때 해당 거래의 자금을 받는 수령자 주소가 정산 이벤트에서 보인다. zkScatter의 Layer 3 claim 회로(§4.4, §5 Phase 5)는 정산 시점에 커밋된 claims 트리에서 수령자의 Merkle 포함을 증명하는 **세 번째 proof 단계**를 삽입하며, 이때 어떤 claim 이 redeem 되는지는 공개하지 않는다. 이것이 computational privacy (Tornado 계열의 트래픽 의존성 및 Jigsaw 계열의 수령자 연결성)를 **트래픽 독립적인 암호학적 프라이버시**로 전환시키는 핵심 분리이다.

2. **Dual-CA 규정 준수 층위.** Jigsaw는 Tornado Cash, Railgun, Renegade, Penumbra 와 마찬가지로 규정 준수/책임성 층위가 없다. zkScatter의 Dual-CA 신원 모델(§4.2, §8)은 의도된 설계이며, 본 출원이 청구하는 "프라이버시와 규제 협력성의 공존을 사용자가 둘 중 하나를 포기하지 않고도 가능하게 하는 메커니즘" 의 본체이다.

3. **다중 수령자 claim 분배.** Jigsaw는 각 측당 단일 은닉 출력으로 정산된다. zkScatter의 settle 회로는 **측당 claims 의 집합**(구현 파라미터 N 에 의해 경계; 참조 회로에서 16 으로 설정 — §13 파라미터화 주석 참고)을 인가하여, 단일 거래로 다수 수령자에게 원자적 fan-out 을 가능하게 한다. §11 의 참조 시나리오(maker 3 claims + taker 1 claim)에서 명시적으로 사용되고 있다.

4. **연합(federated) 릴레이어 책임성.** Jigsaw는 릴레이어 네트워크를 명세하지 않는다. zkScatter 의 공개 신원 기반 연합 릴레이어 모델(§4.5) + record-only 분쟁 레지스트리(§9.6)는 동일 출원의 공동 설계 요소이다 — 이것이 프라이버시 아키텍처를 규제 당국이 추론 가능한 시스템으로 전환시킨다.

본 출원은 Jigsaw를 가장 가까운 선행기술로 **완전히 인지한 상태에서** 제출된다. 위 1번(Layer 3 를 통한 거래-클레임 분리) 및 2번(Dual-CA 결합) 을 커버하는 청구항은 Jigsaw 위에서 특허 가능하도록 충분히 좁게 기재되며, 3번과 4번은 청구항 협소화 요소 자체는 아니지만 설명 상 맥락적 차이로 기재한다.
...
[23] "Jigsaw: A Privacy-Preserving Trade Primitive." Cryptology ePrint Archive, Report 2025/1147, 2025. https://eprint.iacr.org/2025/1147 *(본 출원의 선행기술 조사에서 가장 가까운 선행기술로 식별됨; §10 "가장 가까운 선행기술: Jigsaw [23]" 참고. IDS 제출 전 ePrint 레코드에 대해 저자 명단과 최종 제목을 확인할 것.)*
