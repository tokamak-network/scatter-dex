...
Jigsaw (Garg, Goel, Kolonelos, Sinha; *Jigsaw: Doubly Private Smart Contracts*; Cryptology ePrint Archive Paper 2025/1147; 최초 공개 2025-06-18, 최종 수정 2025-10-15)는 프라이버시-거래 설계 공간에서 본 발명에 가장 가까이 위치한 공개 구성으로, 본 출원의 선행기술 조사에서 **가장 가까운 선행기술(closest prior art)** 로 식별되었다. Jigsaw는 **이중 프라이빗(doubly private) 스마트 컨트랙트** 프레임워크를 제안하며, 온체인 *및* 오프체인 프라이버시를 모두 다룬다. 클라이언트는 프라이버시 보존 방식으로 상호 비신뢰(mutually-untrusting) 서버 그룹에 요청을 제출하고, 이 서버들은 "클라이언트의 데이터나 신원에 대해 어떠한 정보도 학습하지 않고" 요청을 **협력적으로 매칭** 한다 (Jigsaw 초록). 구현은 ZEXE 아키텍처 (Bowe et al., S&P 2020) 위에 구축되며, Collaborative zkSNARKs (Ozdemik과 Boneh, USENIX 2022) 를 확장하여 **서버 그룹에 의한 proof 생성** 을 가능하게 한다. Jigsaw 논문은 탈중앙화 거래소, 경매, 투표를 포함한 샘플 응용에서 이 프레임워크를 시연한다.
