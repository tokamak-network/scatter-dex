...
### 같은 토큰 페어 (sellToken == buyToken) — 시간 지연 분산 송금

거래가 아닌 **프라이버시 전송** 용도:
- 에스크로에서 수신자에게 시간차로 자금 전달
- Vesting, 프라이버시 송금, 예약 지급에 활용
- 자기 자신이 maker + taker가 되어 settle
- 컨트랙트는 sellToken == buyToken을 제한하지 않음
