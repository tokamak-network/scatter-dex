...
Tornado Cash는 법 집행기관에 협력할 수 있는 책임 있는 중개자가 없었기 때문에 OFAC의 제재를 받았다. Renegade는 비용이 많이 드는 다자간 계산을 통해 프라이버시를 달성한다. Railgun은 ZK 프라이버시를 제공하지만 규정 준수 메커니즘이 없다. Jigsaw [23]는 최근에 제안된 프라이버시-거래 원시(primitive)로 본 발명과 가장 가까운 선행기술이며, **상호 비신뢰(mutually-untrusting) 서버 그룹** 위에서 Collaborative zkSNARK 를 확장하여 "이중 프라이빗(doubly private)" (온체인 + 오프체인) 실행을 달성하지만, 책임성 및 규제기관 감사(audit) 층위는 설계 범위 밖에 두고 있고 사용자 측 단일 prover client-side 증명이 아닌 **서버 그룹의 협력 증명** 을 요구한다. 기존 시스템 중 세 가지를 모두 해결하는 것은 없다.
...
- 잔액 충분성
- 측당 최대 **N개의 claim 리프**로부터 claims tree root가 올바르게 계산됨 (N 은 구현 파라미터로서 참조 회로에서 16 으로 설정 — §13 "구현 파라미터" 참고)
...
| 4세대 | Renegade, Railgun, Jigsaw [23] | 프라이버시 우선 (ZK/MPC) |
| **5세대** | **zkScatter** | **ZK commitment 풀 + Dual-CA 규정 준수** |

### 가장 가까운 선행기술: Jigsaw [23]

Jigsaw (Garg, Goel, Kolonelos, Sinha; *Jigsaw: Doubly Private Smart Contracts*; Cryptology ePrint Archive Paper 2025/1147; 최초 공개 2025-06-18, 최종 수정 2025-10-15)는 프라이버시-거래 설계 공간에서 본 발명에 가장 가까이 위치한 공개 구성으로, 본 출원의 선행기술 조사에서 **가장 가까운 선행기술(closest prior art)** 로 식별되었다. Jigsaw는 **이중 프라이빗(doubly private) 스마트 컨트랙트** 프레임워크를 제안하며, 온체인 *및* 오프체인 프라이버시를 모두 다룬다. 클라이언트는 프라이버시 보존 방식으로 상호 비신뢰(mutually-untrusting) 서버 그룹에 요청을 제출하고, 이 서버들은 "클라이언트의 데이터나 신원에 대해 어떠한 정보도 학습하지 않고" 요청을 **협력적으로 매칭** 한다 (Jigsaw 초록). 구현은 ZEXE 아키텍처 (Bowe et al., S&P 2020) 위에 구축되며, Collaborative zkSNARKs (Ozdemir과 Boneh, USENIX 2022) 를 확장하여 **서버 그룹에 의한 proof 생성** 을 가능하게 한다. Jigsaw 논문은 탈중앙화 거래소, 경매, 투표를 포함한 샘플 응용에서 이 프레임워크를 시연한다.

zkScatter가 Jigsaw와 구별되는 지점은 다음과 같으며, 각각은 본 발명의 본질적 구성요소이다:

1. **단일 prover client-side 증명 vs 협력적 multi-server 증명.** Jigsaw 의 proof 는 Collaborative zkSNARK 를 실행하는 **상호 비신뢰 서버 그룹** 에 의해 생성되며, 프라이버시 보장은 그 서버 그룹의 다수가 공모하지 않는다는 가정에 의존한다. zkScatter 는 Half-proof primitive (`circuits/authorize.circom`, [docs/circuit-split/design.md](circuit-split/design.md) 참고) 하에서 **사용자 자신의 device 에서 브라우저 내** 단일 prover 가 모든 증명을 생성한다 — 각 사용자는 자신의 측만 증명하고, 릴레이어는 public proof output 만 보게 된다. 신뢰 가정은 "서버 그룹 간 공모 없음" 에서 "사용자 자신의 local device 가 손상되지 않음" 으로 약화되며, 브라우저/모바일 최종 사용자에게는 압도적으로 배포 가능성이 높고, **이것이 zkScatter 가 Jigsaw 가 의존하는 Collaborative zkSNARK 기계장치가 필요 없는 이유** 이다.

2. **Dual-CA 규정 준수 층위.** Jigsaw 는 Tornado Cash, Railgun, Renegade, Penumbra 와 마찬가지로 통합된 규정 준수/책임성 층위가 없다. 초록은 신원, 감사, 규제 협력성 어느 것도 언급하지 않는다. zkScatter 의 Dual-CA 신원 모델(§4.2, §8)은 본 출원이 청구하는 "사용자 측 프라이버시와 규제 협력성의 공존을 사용자가 둘 중 하나를 포기하지 않고도 가능하게 하는 메커니즘" 의 본체이며, Tornado Cash 의 규제 실패 모드를 다룬다 (§8 "Tornado Cash 문제를 회피하는 이유").

3. **Layer 3 거래-클레임 분리.** Jigsaw 의 협력적 서버 파이프라인은 거래당 *단일* 정산 단계를 생성한다. zkScatter 는 **세 번째**, 별도로 증명 가능한 단계 (§4.4 claim 회로, §5 Phase 5) 를 삽입하여, 정산 시점에 커밋된 claims 트리에서 수령자의 Merkle 포함을 증명하되 **어떤 claim 이 redeem 되는지는 공개하지 않는다**. 이 분리는 수령자 층위에서 computational privacy 를 트래픽 독립적인 암호학적 프라이버시로 전환시킨다 — Jigsaw 를 포함한 어떤 단일 단계 정산 원시에서도 존재하지 않는 속성이다.

4. **연합 릴레이어 책임성 vs 협력적 서버 모델.** Jigsaw 의 "상호 비신뢰" 서버는 liveness/correctness 측면에서만 책임을 진다 (Collaborative zkSNARK 프로토콜이 deviation 을 검출). 규제 기관에 대한 책임성도 없고 온체인 신원도 없다. zkScatter 의 연합 릴레이어 모델 — `RelayerRegistry` 의 공개 Dual-CA 신원, record-only `DisputeRegistry`, 평판 강제 — 은 Jigsaw 가 제공하지 않는 공동 설계된 책임성 층위이다.

5. **다중 수령자 claim 분배 (구현 파라미터, 청구항 요소 아님).** Jigsaw 의 샘플 DEX 응용은 측당 단일 출력으로 정산된다. zkScatter 의 settle / authorize 회로는 **측당 claims 의 집합** (구현 파라미터 N 에 의해 경계; 참조 회로에서 16 으로 설정, §13 파라미터화 주석 참고) 을 인가하여, 단일 거래로 다수 수령자에게 원자적 fan-out 을 가능하게 한다. §11 의 참조 시나리오 (maker 3 claims + taker 1 claim) 에서 사용되고 있다. N 자체는 청구항 협소화 요소가 아니다.

본 출원은 Jigsaw를 가장 가까운 선행기술로 **완전히 인지한 상태에서** 제출된다. 위 1번 (단일 prover client-side 증명, 협력적 서버 요구사항 없음), 2번 (Dual-CA 결합), 3번 (Layer 3 거래-클레임 분리), 4번 (연합 릴레이어 책임성) 을 커버하는 청구항은 각각 Jigsaw 위에서 특허 가능하도록 충분히 좁게 기재된다. 5번은 구현 특성으로만 기재한다.
...
### 구현 파라미터 (청구항 범위 제한 요소 아님)

다음 수치 값들은 참조 구현과 본 명세서 전반에 걸쳐 등장한다. 이들은 **구현 파라미터** 이며 본 발명의 청구항 범위를 제한하려는 의도가 아니다. 통상의 기술자는 본 발명의 핵심 개념(Layer 1/2/3 분리, Dual-CA 신원, ZK commitment 풀, nullifier 기반 이중지출 방지, 연합 릴레이어 책임성)을 벗어나지 않고 아래 모든 값을 변경할 수 있으며, 아래 표의 "자유 변경 가능" 으로 표시된 항목의 변경은 본 출원의 범위 내에 포함되며 계속 출원(continuation filing)을 요하지 않는다.

| 파라미터 | 참조 값 | 설정 위치 | 발명 변경 없이 허용되는 변형 |
|---|---|---|---|
| **N** — 단일 정산에서 측당 최대 claim 수 | **16** | `settle.circom` / `authorize.circom` — `maxClaimsPerSide = 16`, claims 트리 깊이 `ceil(log2(N)) = 4` | 2 의 거듭제곱 `N ∈ {2, 4, 8, 16, 32, 64, …}` 중 임의 값이 유효하다. `N` 변경은 회로 재생성과 trusted setup 재실행을 필요로 하지만, 새로운 발명 단계를 도입하지 않는다. `N = 16` 은 §11 의 참조 시나리오(maker 3 + taker 1 claim = 측당 4)가 여유 있게 수용되고, 결과 claims 트리 깊이(4)가 30K 제약 전체에 비해 무시할 수 있기에 선택되었다. 배치-분배(batch-distribution) 용례를 겨냥한 향후 프로덕션 배포는 `N = 32` 또는 `N = 64` 를 선택할 수 있으며, 본 출원은 이러한 변형 전체를 포함하려는 의도이다. |
| **D** — commitment Merkle 트리 깊이 | **20** (용량 2²⁰ ≈ 1M commitment) | `CommitmentPool` / `settle.circom` — `commitTreeDepth = 20` | `D ∈ {16, 20, 24, 28, 32}` 중 임의 값이 유효하며, 트리 용량과 삽입당 제약 비용(insert 당 ≈ D 회 Poseidon 해시) 간의 trade-off 이다. `D = 20` 은 풀당 ~1M 의 누적 commitment 지원을 위해 선택되었으며, Tornado Cash 의 참조 깊이와 일치하고 30K 제약 예산 안에서 편안히 수용된다. |
| **ROOT_HISTORY_SIZE** — 온체인 ring buffer 에 보관되는 과거 root 수 | **30** (참조) | `IncrementalMerkleTree.sol` 생성자 인자 | 임의의 양의 정수가 유효하다. 값이 커지면 비동기 매칭 윈도우가 길어지고(§circuit-split), 작아지면 해당 수의 스토리지 슬롯을 절약한다. 배포 시점의 선택 사항이다. |
| **금액 범위 검증 비트폭** | **126 비트** | `settle.circom` / `authorize.circom` — `sell/buy amounts` 에 `Num2Bits(126)` | **이 값은 자유롭게 변경할 수 없다** — `docs/circuit-split/bit-width-audit.md` 참고. 126 비트는 252 비트 가격 곱이 `LessEqThan(252)` 내부 표현에 BN254 스칼라 필드를 감싸지 않고 들어맞는 최대 폭이다. 더 좁은 값(예: 125 비트)은 안전하고, 더 넓은 값(127 비트 이상)은 안전하지 않다. 이는 파라미터 선택이 아닌 정확성 경계이다. |
| **수수료 bps 정밀도** | **16 비트** (최대 65535, 100% = 10000) | `settle.circom` — `makerFee`, `takerFee` 에 `Num2Bits(16)` | 126 비트 이하의 임의 폭이 안전하다 (126-비트 금액과 곱했을 때 충분한 여유를 남김). DeFi 수수료 체계에서 관례적으로 사용되는 bp 인코딩과 일치하기에 16 비트가 선택되었다. |
| **REVEAL_WINDOW** — fair-exchange reveal 마감 | **5 분** | `relayer-protocol/design.md` §5.4, §13 | 임의의 양의 지속시간이 유효하다. `DisputeRegistry.REVEAL_WINDOW` 와 일치해야 한다. 윈도우가 짧으면 처리량이 개선되지만 릴레이어의 타이밍 예산이 빠듯해지고, 길면 네트워크 지터에 더 관대해진다. |

**범위 주장**: 본 발명은 §3-§9 에 기재된 아키텍처 요소들의 결합(3계층 분리, Dual-CA 신원, ZK commitment 풀, nullifier 이중지출 방지, 연합 릴레이어 책임성)에 의해 정의된다. 위 표의 수치 값들은 참조 구현의 재현성을 위해 제공되며, "자유 변경 가능" 으로 표시된 행에 대해 다른 값을 사용하는 실시예(embodiment)도 본 명세서의 범위에 포함된다. "자유 변경 불가" 로 표시된 행(126 비트 금액 범위 검증) 만이 파라미터 선택이 아닌 정확성 경계이며, 이조차도 본 발명의 핵심 개념이 아닌 **대상 타원곡선(BN254)의 특성에 기인** 한 것이다 — 다른 곡선(BLS12-381, BW6-761 등)을 대상으로 하는 구현은 다른 정확성 경계를 가질 것이다.
