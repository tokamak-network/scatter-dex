...
> - `PAPER.md` §10 "Closest Prior Art: Jigsaw [23]" — the narrative disclosure of the Jigsaw reference in the specification (provenance citation; the PAPER drafts were removed from the repo — recover from git history. Successor spec: [../../developers/docs/whitepaper.mdx](../../developers/docs/whitepaper.mdx))
> - `PAPER-ko.md` §10 "가장 가까운 선행기술: Jigsaw [23]" — Korean mirror (same provenance note)
> - [../architecture/architecture-v2.md](../architecture/architecture-v2.md) — Design decisions that distinguish over the references below
>
