...
[취소 후]   E, N 은 소진됨 → relayer 가 가진 authorize 증명은 이제 무효
            C' 라는 새 커밋먼트(같은 secret·잔액·pubKey, 새 salt)가 트리에 삽입됨
    ▼ 사용자가 C' 로 새 authorize 증명 생성 (salt 가 바뀌어 nullifier 도 새 값)
[새 주문]   escrowNullifier=E' (새 salt 에서 파생), nonceNullifier=N' (새 nonce)
```
...
mapping(bytes32 => ClaimsGroup) public claimsGroups;// claimsRoot → {totalLocked, totalClaimed, token, tier}
```
...
| `claimCount` | 8-bit (0–`maxClaimsPerSide`) | 티어 16/64/128 모두 커버 (PR #604 — 구 5-bit 검사는 상위 티어에서 오버플로) |

> 세부 감사 내역: `docs/circuit-split/bit-width-audit.md` (docs 재구성 때 삭제 — git 히스토리에서 열람).
