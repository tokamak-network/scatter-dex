...
| 카테고리 | 컨트랙트 | 라인 | 역할 |
|----------|----------|------|------|
| 핵심 | `zk/CommitmentPool.sol` | 390 | 예치·출금·Merkle tree 관리, 세틀먼트용 자금 이동 |
| 핵심 | `zk/PrivateSettlement.sol` | 1,113 | 4종 세틀먼트 플로우, Claims Group, Claim 실행 |
| 트리 | `zk/IncrementalMerkleTree.sol` | 118 | Poseidon 기반 append-only 트리 + 루트 링 버퍼 |
| 라이브러리 | `zk/SettleVerifyLib.sol` | 278 | EIP-170 회피용 외부 라이브러리(pure/view helpers) |
| Verifier | `zk/{Deposit,Withdraw,Settle,Authorize,Claim,Cancel,BatchAuthorize}Verifier.sol` | auto-gen | snarkJS Groth16 검증기 |
| 수수료 | `FeeVault.sol` | 199 | 릴레이어 수수료 + 플랫폼 수익 분리 적립 |
| 릴레이어 | `RelayerRegistry.sol` | 208 | 릴레이어 등록/본드/7일 쿨다운 |
| 컴플라이언스 | `IdentityGate.sol` | 131 | 다중 zk-X509 CA OR 집계 게이트 |
| 컴플라이언스 | `SanctionsList.sol` | 74 | OFAC 스타일 블록리스트 |
| UX | `BatchExecutor.sol` | 90 | EIP-7702 최소 배치 실행기(ERC-7579 호환) |

## 2. 세부 문서

- [core-contracts.md](core-contracts.md) — **CommitmentPool**, **PrivateSettlement** 상세
- [supporting-contracts.md](supporting-contracts.md) — FeeVault, RelayerRegistry, IdentityGate, SanctionsList, BatchExecutor, IncrementalMerkleTree, SettleVerifyLib, Verifier 계열
...
8. **10% 풀 수수료 캡** — `transferFee` 가 풀 잔액의 10% 초과 시 revert.
