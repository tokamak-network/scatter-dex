# zkScatter 컨트랙트 설계 문서

> 본 문서는 zkScatter 온체인 컨트랙트 시스템의 설계를 한글로 정리한 문서입니다.
> 증명(ZK) 관련 상세 설명은 [../proof-system/README.md](../proof-system/README.md) 참조.

## 1. 시스템 개요

zkScatter 는 **ZK 기반 프라이빗 정산 네트워크**(private settlement network — DEX 가 아니다, [화이트페이퍼](../../../developers/docs/whitepaper.mdx) §0 참조)로, 다음 네 가지 축으로 구성된다.
...
└───────┬──────────┘                 └──────┬────────────────┘
        ...
        ▼                                   ▼
┌──────────────────┐                 ┌───────────────────────┐
│  Deposit /       │                 │  Authorize / Claim /  │
│  Withdraw Verif. │                 │  Cancel / BatchAuth.  │
│  (Groth16)       │                 │  Verif. (티어별 레지스트리)│
└──────────────────┘                 └───────────────────────┘
                                            │
              ┌─────────────────────────────┼─────────────────────────────┐
              ▼                             ▼                             ▼
...
| Verifier | `zk/{Deposit,Withdraw,Authorize,Claim,Cancel,BatchAuthorize}Verifier.sol` (+ `Authorize/Claim` 의 `_64`/`_128` 티어판) | snarkJS Groth16 검증기. Authorize/Claim/BatchAuthorize 는 티어(16/64/128)별 레지스트리로 디스패치 |
...
5. **타임락 & 2-step 오너십** — 세틀먼트 컨트랙트 교체 24시간, 플랫폼 수수료 변경 기본 1일 지연(owner 가 최대 30일까지 조정 가능).
