...
> **플랫폼**: Android + iOS, Expo 54 + React Native 0.81 + Hermes + 네이티브 증명자(Rust, mopro-ffi)
...
> **이력**: 초기 구현은 숨김 WebView(snarkjs WASM) + postMessage 브리지였다.
> Phase C-4/C-5 에서 WebView 증명 경로와 폴백을 전부 제거하고 **100% 네이티브
> Rust 증명자(mopro-ffi)** 로 전환했다 — `ZKBridgeService` 는 네이티브 export
> 의 얇은 래퍼로만 남아 있다 (`mobile/src/services/ZKBridgeService.ts`,
> `NativeProverService.ts` 헤더 주석 참조).

```
Hermes (RN)                        네이티브 모듈 (Rust)
  ethers 6                            mopro-ffi (Groth16 prover)
  WalletConnect / built-in            circuits/build 의 zkey 소비
  UI + 상태                           uniffi 바인딩
      │                                   │
      └──── NativeProverService.generateNativeProof ────┘
```

- **모든 회로 소비 경로가 `NativeProverService.generateNativeProof` 를 통과**한다 — WebView 폴백 없음.
- **zkey 번들링** (`mobile/scripts/copy-zk-assets.sh`): `circuits/build/` 의 zkey 를 `mobile/assets/zk-native/` 로 복사하고 expo-asset 이 인덱싱, 런타임에 네이티브 증명자가 로드.

### 10.1 회로 티어 정책 — 모바일은 TIER_16 전용

웹 프론트엔드(apps/pay, apps/pro)는 멀티-티어 인프라(TIER_16 / TIER_64 / TIER_128)를 지원하고 `pickActiveTier(recipientCount)`로 자동 라우팅하지만, **모바일은 TIER_16만 번들링**합니다(네이티브 증명자 기준 — WebView 경로는 존재하지 않음).
