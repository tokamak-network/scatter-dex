/* zkScatter Hub brandmark.
 *
 * Composite of the Tokamak Network symbol with a centred italic-serif
 * "zk" overlay, followed by the "Scatter" wordmark. Mirrors the docs
 * and pro brandmarks for cross-app consistency.
 *
 * DRIFT: paths below are duplicated from the shared brand and from
...
/* Two click targets: the symbol itself routes to the cross-app hub
 * (so users always have a way back from any sub-app) while the
 * wordmark routes to the current site's home. Hub passes the same
 * URL for both, since "current site" *is* the hub. */
const HUB_HREF =
