...
// (`authorize.circom`, `authorize_64.circom`, `authorize_128.circom`)
// instantiate `Authorize(commitTreeDepth, maxClaimsPerSide,
// claimsTreeDepth)` with their tier's parameters so each tier compiles
// into its own r1cs / wasm / zkey under `circuits/build/`. Adding a
// new tier = one more wrapper that includes this file.
//
// NOTE: pubKeyBind is a `signal output` inside Authorize, so it is
// automatically public in circom 2.x. In every tier's verifier
// pubSignals array it appears first (index 0), before commitmentRoot
// and the other explicitly listed public signals.
