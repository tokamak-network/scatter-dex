...
export const GnarkProofResult = (() => {
  const defaults = () => ({});
  const create = (() => {
    return uniffiCreateRecord<GnarkProofResult, ReturnType<typeof defaults>>(
      defaults
    );
  })();
  return Object.freeze({
...
export const MoproError = (() => {
  ...
  class GnarkError_ extends UniffiError implements GnarkError__interface {
    ...
    static getInner(obj: GnarkError_): Readonly<[string]> {
      return obj.inner;
    }
  }
  ...
  class FFIConverter extends AbstractFfiConverterByteArray<TypeName> {
    read(from: RustBuffer): TypeName {
      switch (ordinalConverter.read(from)) {
        ...
        case 4:
          return new MoproError.GnarkError(FfiConverterString.read(from));
        default:
...
function uniffiEnsureInitialized() {
  ...
  if (
    ...
    throw new UniffiInternalError.ApiChecksumMismatch(
      'uniffi_native_prover_checksum_func_mopro_hello_world'
    );
  }
  if (
