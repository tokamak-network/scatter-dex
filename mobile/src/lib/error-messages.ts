 ...
 *
 * Mirrors `frontend/app/lib/error-messages.ts` — keep the two files in
 * sync so the same raw Solidity revert / ethers error / ZK proof
 * failure surfaces the same copy on web and mobile.
 */
...
// ethers.js / wallet error patterns
const WALLET_PATTERNS: Array<[RegExp, string]> = [
  ...
  [/network.*changed|chain.*mismatch/i, 'Wrong network. Please switch to the correct chain in Settings.'],
  ...
  [/could not detect network/i, 'Cannot connect to the blockchain. Check the RPC URL in Settings.'],
  [/missing revert data/i, 'Transaction would fail. The contract rejected this operation.'],
];

// ZK / proof errors — copy diverges from web only where the remediation
// is platform-specific ("Restart the app" vs "Refresh the page").
const ZK_PATTERNS: Array<[RegExp, string]> = [
  [/wasm.*not.*found|wasm.*failed/i, 'ZK proof engine failed to load. Restart the app and try again.'],
  [/witness.*failed|constraint.*not.*satisfied/i, 'ZK proof generation failed. Your input values may be invalid.'],
  [/zkey.*not.*found/i, 'ZK proving key not found. Reinstall the app or restore from backup.'],
...
// Pre-compile regex patterns for contract errors (avoid recreating per call).
 ...
 * Returns the friendly message, or the original message if no match is
 * found. Logs the full error for debugging.
 */
export function friendlyError(error: unknown): string {
  ...
  // 4. Truncate long raw messages so Alert bodies stay legible.
