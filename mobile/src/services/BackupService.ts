...
export const BackupService = {
  ...
  async restore(bundle: BackupBundle): Promise<RestoreSummary> {
    ...
    // tuple (txHash, leafIndex, amount) which is stable per-order. New
    // `id`s are generated by `append`, so a re-import gets fresh ids and
    // we dedup on content. Validate the row shape up front so a malformed
    // entry can't cause `append` to throw mid-write — that would leave
    // orphan secrets/meta after some have already landed.
