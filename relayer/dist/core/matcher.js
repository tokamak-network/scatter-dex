...
export class Matcher {
    ...
    findMatch(newOrder) {
        ...
        for (const candidate of candidates) {
            if (candidate === newOrder)
                continue;
            if (candidate.status !== "pending")
                continue;
            if (candidate.order.expiry <= now)
                continue;
            if (candidate.order.maker.toLowerCase() === order.maker.toLowerCase())
                continue;
            // Token check
            if (candidate.order.sellToken.toLowerCase() !== order.buyToken.toLowerCase())
                continue;
            if (candidate.order.buyToken.toLowerCase() !== order.sellToken.toLowerCase())
                continue;
            // Price compatibility (matches Solidity: maker.sell * taker.sell <= maker.buy * taker.buy)
            const compatible = order.sellAmount * candidate.order.sellAmount <=
                order.buyAmount * candidate.order.buyAmount;
            if (!compatible)
                continue;
            // Amount compatibility: each side has enough to fill the other
            if (candidate.order.sellAmount < order.buyAmount)
                continue;
            if (order.sellAmount < candidate.order.buyAmount)
                continue;
