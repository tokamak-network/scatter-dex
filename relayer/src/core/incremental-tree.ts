...
export class IncrementalMerkleTree {
  ...
  /** Insert a leaf. O(depth). Returns the leaf index. */
  async insert(leaf: bigint): Promise<number> {
    const capacity = 2 ** this.depth;
    if (this._leaves.length >= capacity) throw new Error("tree full");
  ...
  async getProofAsync(leafIndex: number): Promise<{ pathElements: bigint[]; pathIndices: number[] }> {
    ...
    let currentLayer = padded;

    for (let i = 0; i < this.depth; i++) {
      const nextLayer: bigint[] = [];
      for (let j = 0; j < currentLayer.length; j += 2) {
        nextLayer.push(await poseidonHash([currentLayer[j], currentLayer[j + 1]]));
      }
  ...
  /** Rebuild tree from existing leaves (startup recovery). */
