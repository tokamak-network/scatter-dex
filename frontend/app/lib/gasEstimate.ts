...
export async function estimateMinFeeBps(
  ...
  if (sellAmount <= 0n || !ethPerToken || ethPerToken <= 0) {
  ...
  // Convert ETH gas cost → token amount
  // totalGasWei is in wei (1e18 = 1 ETH)
  // ethPerToken = how many ETH per 1 token (e.g., 0.0005 for USDC at $2000/ETH)
  // tokenPerEth = 1 / ethPerToken
  // gasCostToken = totalGasWei * tokenPerEth / 1e18
  //              = totalGasWei / (ethPerToken * 1e18) * 10^tokenDecimals
  const tokenPerEth = 1 / ethPerToken;
  const gasCostEth = Number(totalGasWei) / 1e18;
  const gasCostTokenFloat = gasCostEth * tokenPerEth;
  const gasCostToken = BigInt(Math.ceil(gasCostTokenFloat * 10 ** tokenDecimals));

  // minFeeBps = ceil(gasCostToken * 10000 / sellAmount)
  const minFeeBps = Number((gasCostToken * 10000n + sellAmount - 1n) / sellAmount);
