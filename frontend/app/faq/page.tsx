...
const faqs: { icon: React.ReactNode; category: string; q: string; a: string }[] = [
  {
              ...
              {items.map((faq) => (
                ...
                >
                  <summary className="flex items-center justify-between cursor-pointer px-8 py-5 text-on-surface font-headline font-semibold text-lg select-none list-none [&::-webkit-details-marker]:hidden">
                    {faq.q}
                    <span className="ml-4 shrink-0 w-6 h-6 flex items-center justify-center rounded-full bg-surface-bright text-on-surface-variant text-sm transition-transform group-open:rotate-45">
                      +
                    </span>
                  </summary>
