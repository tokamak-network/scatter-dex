...
function isTransientSendError(err: unknown): boolean {
  ...
  const permanentPatterns = [
    ...
    "execution reverted",
    "insufficient funds",
    "nonce too low",       // already mined with this nonce
    "replacement fee too low", // need higher gas, but same nonce already pending
    "invalid argument",
    "invalid address",
    "unpredictable_gas_limit", // ethers: call would revert
  ];
...
export async function sendAndWait(
  ...
  if (!receipt) {
    // Timeout — TX is in mempool but not yet mined. Poll by hash.
    console.warn(
...
async function waitWithTimeout(
  ...
  // Suppress unhandled rejection from the losing branch of the race.
