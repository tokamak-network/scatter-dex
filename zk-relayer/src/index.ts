...
async function main() {
  ...
  // Diagnostic — opt-in, mounted *before* `express.json` so the
  // body-stream chunk listeners can actually fire (the JSON parser
  // consumes the stream end-to-end, so a same-method middleware
  // mounted later sees no `data` events). Gated behind
  // `DIAG_AUTH_ORDERS=1` so production deployments don't carry the
  // per-request listener overhead. See PR #414/#418 for the
  // investigation that justified adding it.
