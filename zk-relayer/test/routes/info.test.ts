...
describe("/api/info", () => {
  it("GET / returns relayer metadata", async () => {
    const app = mountRouter(
      "/api/info",
      createInfoRoutes(makeOrderbookStub({ getOrderCount: () => 7 }), makeSubmitterStub()),
    );
  ...
  it("GET /merkle-proof returns proof for valid leafIndex", async () => {
    const app = mountRouter("/api/info",
      createInfoRoutes(makeOrderbookStub(), makeSubmitterStub()));
  ...
  it("GET /merkle-proof returns 500 when submitter throws", async () => {
    ...
    const app = mountRouter("/api/info",
      createInfoRoutes(makeOrderbookStub(), submitter));
