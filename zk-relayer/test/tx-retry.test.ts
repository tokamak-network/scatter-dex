...
function mockTxResponse(hash: string) {
  return { hash, wait: vi.fn() };
}

function mockProvider() {
  return { getTransactionReceipt: vi.fn() };
}
...
describe("sendAndWait", () => {
  ...
  it("retries transient send errors", async () => {
    ...
    const result = await sendAndWait(sendFn, mockProvider() as any, {
      label: "test",
      sendRetries: 3,
      sendRetryBaseMs: 10,
    });
  ...
  it("does NOT retry permanent send errors (revert)", async () => {
    ...
    await expect(
      sendAndWait(sendFn, mockProvider() as any, {
        label: "test",
        sendRetries: 3,
        sendRetryBaseMs: 10,
      }),
  ...
  it("polls receipt when wait times out", async () => {
    ...
    provider.getTransactionReceipt
      ...
      .mockResolvedValueOnce({ hash: "0xslow", status: 1 });
