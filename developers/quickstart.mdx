---
title: Quickstart
description: "From npm install to a settled private trade — end to end."
---

This walkthrough wires the SDK into a fresh Next.js app and exercises
every layer: wallet → deposit → order → settle → claim. Skim the
[Architecture](/concepts/architecture) page first if any of those words
look unfamiliar.

## 1. Install

```bash
npm install @zkscatter/sdk ethers react@^19
```

See [Installation](/installation) for Next.js wiring and Web Worker
asset setup.

## 2. Configure the network

```ts lib/network.ts
import {
  parseTokenList,
  withNativeEthAlias,
  type NetworkConfig,
} from "@zkscatter/sdk";

const wethAddress = "0x...";

export const network: NetworkConfig = {
  chainId: 11155111,
  rpcUrl: process.env.NEXT_PUBLIC_RPC_URL!,
  contracts: {
    privateSettlement: "0x...",
    commitmentPool: "0x...",
    relayerRegistry: "0x...",
    identityGate: "0x...",
    feeVault: "0x...",
    weth: wethAddress,
  },
  tokens: withNativeEthAlias(
    parseTokenList(process.env.NEXT_PUBLIC_TOKEN_LIST),
    wethAddress,
  ),
};
```

`parseTokenList` expects the compact `addr:symbol:decimals,…` format —
useful for shipping the same config through env vars across web and mobile.

## 3. Mount the wallet provider

```tsx app/providers.tsx
"use client";

import { WalletProvider } from "@zkscatter/sdk/react";
import { network } from "@/lib/network";

export function Providers({ children }: { children: React.ReactNode }) {
  return <WalletProvider network={network}>{children}</WalletProvider>;
}
```

```tsx app/layout.tsx
import { Providers } from "./providers";

export default function RootLayout({ children }: { children: React.ReactNode }) {
  return (
    <html lang="en">
      <body><Providers>{children}</Providers></body>
    </html>
  );
}
```

## 4. Deposit into the pool

```tsx components/Deposit.tsx
"use client";

import { useWallet } from "@zkscatter/sdk/react";
import {
  generateNote,
  generateDepositProof,
  deriveEdDSAKey,
} from "@zkscatter/sdk/zk";
import { ensureAllowance, callDeposit } from "@zkscatter/sdk/contracts";
import { network } from "@/lib/network";

export function DepositButton({ token, amount }: { token: string; amount: bigint }) {
  const { signer } = useWallet();

  async function deposit() {
    if (!signer) throw new Error("Connect wallet first");

    const eddsa = await deriveEdDSAKey(signer);
    const note = generateNote(token, amount, eddsa.publicKey);

    const result = await generateDepositProof(note, {
      wasm: "/zk/deposit.wasm",
      zkey: "/zk/deposit_final.zkey",
    });

    await ensureAllowance(signer, token, network.contracts.commitmentPool, amount);
    const tx = await callDeposit(
      signer,
      network.contracts.commitmentPool,
      result,
      token,
      amount,
    );
    await tx.wait();

    // Persist `note` + `result.commitment` — see /guides/persist-notes.
  }

  return <button onClick={deposit}>Deposit</button>;
}
```

## 5. Discover relayers and place an order

```ts
import {
  loadRelayersWithApiInfo,
  RelayerClient,
} from "@zkscatter/sdk/relayer";
import { generateAuthorizeProof } from "@zkscatter/sdk/zk";

const relayers = await loadRelayersWithApiInfo(
  network.contracts.relayerRegistry,
  readProvider,
);
const best = relayers.find((r) => r.online && r.api);
if (!best?.api) throw new Error("No relayer online");

const authResult = await generateAuthorizeProof(
  {
    note,
    leafIndex,
    merkleProof,
    sellToken: tokenA,
    buyToken: tokenB,
    sellAmount: amountIn,
    buyAmount: amountOut,
    maxFee,
    expiry: Math.floor(Date.now() / 1000) + 600,
    nonce,
    relayer: best.address,
    eddsaPrivateKey: eddsa.privateKey,
    claims: [/* stealth-addressed payouts */],
    newSalt: randomFieldElement(),
  },
  { wasm: "/zk/authorize.wasm", zkey: "/zk/authorize_final.zkey" },
);

const client = new RelayerClient(best.url);
await client.submitOrder(orderData, signature, "cover_taker");
```

See [Place an order](/guides/place-order) for the complete flow,
including how to build the merkle proof from indexed pool events.

## 6. Claim a payout

When a counterparty's order matches yours, your stealth payout lands as
a new commitment in the pool. Claim it whenever you like:

```ts
import { generateClaimProof } from "@zkscatter/sdk/zk";
import { callClaimWithProof } from "@zkscatter/sdk/contracts";

const claimResult = await generateClaimProof(claimInput, {
  wasm: "/zk/claim.wasm",
  zkey: "/zk/claim_final.zkey",
});

const tx = await callClaimWithProof(
  signer,
  network.contracts.privateSettlement,
  claimResult.proof,
  { recipient, token, amount, releaseTime },
);
await tx.wait();
```

Claims can also be batched — see [Claim a payout](/guides/claim).

## What's next

<CardGroup cols={2}>
  <Card title="Architecture" icon="diagram-project" href="/concepts/architecture">
    Why half-proofs, why a commitment pool, why stealth addresses.
  </Card>
  <Card title="Persist notes" icon="database" href="/guides/persist-notes">
    Keep notes safe across reloads and devices.
  </Card>
  <Card title="Run a relayer" icon="server" href="/operate/run-a-relayer-node">
    Match orders, charge fees, manage your bond.
  </Card>
  <Card title="SDK reference" icon="code" href="/sdk/overview">
    Every export, every signature.
  </Card>
</CardGroup>
