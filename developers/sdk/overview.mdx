...
- All async APIs accept an optional `AbortSignal` (`signal?` parameter)
  for cancellation.
- ZK and contract-layer numeric values use native `bigint` (commitments,
  amounts in `callDeposit`, EdDSA coordinates, etc.). HTTP wire types
  in `relayer` and `orderbook` use strings for amounts/nonces and
  numbers for `maxFee`/`expiry`/`nonce` to match the relayer protocol.
  Storage adapters serialize bigints at their boundary.
