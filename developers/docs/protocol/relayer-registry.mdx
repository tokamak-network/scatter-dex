...
The current registry **does not implement slashing**. Bonds are
posted on registration and returned in full on exit (after the
7-day cool-down) — there is no `slash()` entry point and no
governance hook to forfeit bond. If a slashing mechanism is needed
(verifiable misbehaviour, censorship penalties), it would land in a
