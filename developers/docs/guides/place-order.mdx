...
const authResult = await generateAuthorizeProof(
  {
    ...
    // `recipient` is the address that will be able to claim this
    // payout. Pass either:
    //   - a regular wallet address (account.toLowerCase()) when the
    //     recipient prefers an audit trail or doesn't need unlinkability,
    //   - a stealth address derived via `generateStealthAddress(meta)`
    //     when unlinkability matters.
    // The `claim` circuit verifies spending-key knowledge either way.
    claims: [
      {
...
entry above (regular or stealth). See [Claim a payout](/docs/guides/claim).
