...
const authResult = await generateAuthorizeProof(
  {
    ...
    // `recipient` is the wallet address that will be able to claim
    // this payout. The `claim` circuit binds the payout to this
    // address — only that EOA can submit the claim.
    claims: [
      {
