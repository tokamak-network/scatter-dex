...
<CardGroup cols={2}>
  ...
  <Card title="Bond posture" icon="server">
    Current bond, slashing history, headroom over `minBond`.
  </Card>
</CardGroup>
...
const filter = contract.filters.TradeSettled(null, null, null, null, null, operatorAddress);
contract.on(filter, (makerNull, takerNull, makerCmt, takerCmt, claimsRoot, relayer, makerFee, takerFee) => {
  // append to UI
});
```

Filtering by the operator address as the 6th `TradeSettled` indexed
arg gives you the firehose of your own settlements.
...
function BondPosture({ row, minBond }: { row: OperatorRow; minBond: bigint }) {
  ...
  return (
    <div>
      ...
      {ratio < 1.5 && <Warning>Top up — single slash could drop you below minimum</Warning>}
    </div>
  );
}
```
