 ...
 *
 *  The set of recognised columns is `name | address | amount | email
 *  | releaseAt`. Apps that don't expose `releaseAt`/`email` pass a
 *  `columns` filter — the parser still recognises the column in the
 *  file but drops its values out of the result and surfaces a
 *  warning so the operator notices their data was ignored.
 */
import { isAddress } from "ethers";
import type { ParsedRecipient, ParseResult, RecipientField } from "./types";

type ColumnKind = "name" | "address" | "amount" | "email" | "releaseAt";
...
const HEADER_PATTERNS: Record<ColumnKind, RegExp> = {
  ...
  // `claim from` mirrors Pro's UI copy; `release` / `releaseAt` /
  // `시작` / `시작일시` cover localised variants and the CSV-style
  // header users hand-edit when round-tripping through textareas.
  releaseAt: /^(release|release[\s_-]?at|claim[\s_-]?from|claimable[\s_-]?from|시작|시작일시|예약시각|release[\s_-]?time)$/i,
};
...
function looksLikeDateTime(s: string): boolean {
  // Accept anything Date.parse handles — covers ISO 8601, RFC 2822,
  // and most Excel-roundtripped strings. We don't tighten further
  // because the host app's `<input type="datetime-local">` does the
  // strict format normalization downstream.
  if (!s) return false;
...
function rowsFromMatrix(
  matrix: unknown[][],
  enabled: ReadonlySet<RecipientField>,
): ParseResult {
  ...
  if (!hasAddrInRow0 && namedCount >= 2) {
    ...
    // (name / address / amount). Optional email / releaseAt columns
    // are header-only — without a header we have no way to tell
    // column 4/5 from a stray data column.
    cols.name = 0;
    cols.address = 1;
    cols.amount = 2;
  }
  ...
  // Surface a warning when the file carries a column the host app
  // didn't opt into — silently dropping `email` from a payroll
  // upload would look like a bug to the operator.
  const ignored: ColumnKind[] = [];
  for (const k of ["name", "email", "releaseAt"] as const) {
    if (cols[k] >= 0 && !enabled.has(k)) ignored.push(k);
  }
  if (ignored.length > 0) {
    warnings.push(
      `Ignored column(s) not enabled in this context: ${ignored.join(", ")}.`,
    );
  }
  ...
  for (let i = dataStart; i < matrix.length; i++) {
    ...
    const email = enabled.has("email") ? get(cols.email) : "";
    if (email && !looksLikeEmail(email)) {
      skippedBadEmail++;
      continue;
    }
    const releaseAt = enabled.has("releaseAt") ? get(cols.releaseAt) : "";
    if (releaseAt && !looksLikeDateTime(releaseAt)) {
      skippedBadDate++;
      continue;
    }
 ...
 */
/** Synchronous CSV-text parser exposed for live textarea editing —
 *  `parseRecipientFile` handles file uploads (xlsx is async). */
export function parseCsv(
  text: string,
  columns: readonly RecipientField[] = ["name", "address", "amount", "email"],
): ParseResult {
  return parseCsvText(text, new Set(columns));
}

function parseCsvText(text: string, enabled: ReadonlySet<RecipientField>): ParseResult {
  const rows: string[][] = [];
  let cur = "";
  let row: string[] = [];
  let inQuotes = false;
  ...
  return rowsFromMatrix(rows, enabled);
}
...
export async function parseRecipientFile(
  ...
  if (ext === "xlsx") {
    ...
    const result = rowsFromMatrix(target.data as unknown[][], enabled);
    if (sheets.length > 1) {
  ...
  if (ext === "csv" || (!ext && file.type === "text/csv")) {
    ...
    return parseCsvText(text, enabled);
  }
  if (ext === "xls") {
    return {
      rows: [],
