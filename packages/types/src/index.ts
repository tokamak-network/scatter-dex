 ...
 * Contains NO secrets (ownerSecret, salt, balance, EdDSA private key,
 * EdDSA public key coordinates, claims).
 *
 * Privacy: the public key coordinate `pubKeyAx` is an owner-identifying
 * handle — exposing it here would correlate every order from the same
 * user. Cross-relayer matching instead uses an opaque `offerHandle`
 * that the maker's relayer resolves internally.
 */
export interface OrderSummary {
  /** Opaque public identifier on the shared orderbook. Generated by the
   *  maker's relayer per order (e.g. random 32-byte hex). Unlinkable
   *  to the user's trading key. Used as the `id` for lookups and as
   *  the target in `TradeOfferRequest.offerHandle`. */
  id: string;
