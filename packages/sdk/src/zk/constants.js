...
export const TIER_16 = { cap: 16, claimsTreeDepth: 4 };
...
export const TIERS = [TIER_16, TIER_64, TIER_128];
...
export const ACTIVE_TIERS = [TIER_16];
...
export function pickTier(recipientCount) {
    if (!Number.isInteger(recipientCount) || recipientCount <= 0) {
        throw new Error(`pickTier: recipientCount must be a positive integer (got ${recipientCount})`);
    }
    for (const tier of TIERS) {
        if (recipientCount <= tier.cap)
            return tier;
    }
    throw new Error(`pickTier: ${recipientCount} recipients exceeds the largest tier (${TIERS[TIERS.length - 1].cap}). ` +
...
export function padClaims(claims, tier, dummy) {
    if (claims.length > tier.cap) {
        throw new Error(`padClaims: ${claims.length} claims exceeds tier ${tier.cap} capacity`);
    }
    const padded = claims.slice();
    while (padded.length < tier.cap)
        padded.push(dummy);
    return padded;
}
