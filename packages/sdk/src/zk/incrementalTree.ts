/** Incremental Poseidon Merkle Tree — mirrors
 *  `IncrementalMerkleTree.sol` for O(depth) insertions. Apps maintain
 *  one of these in memory, fed by `CommitmentInserted` events from
 *  the on-chain pool, so the spend path can supply a real Merkle
 *  proof without rebuilding 2^depth leaves on every action.
 *
 *  Ported from `frontend/app/lib/zk/incremental-tree.ts`. The
 ...
 *  right sibling for the not-yet-filled subtrees during insertion. */
...
export class IncrementalMerkleTree {
  ...
  constructor(depth: number) {
    if (depth > ZEROS.length - 1) {
      throw new Error(
        `IncrementalMerkleTree: depth ${depth} exceeds precomputed zeros (max ${ZEROS.length - 1})`,
      );
    }
    this.depth = depth;
  ...
  async insert(leaf: bigint): Promise<number> {
    if (this._leaves.length >= 2 ** this.depth) {
      throw new Error("IncrementalMerkleTree: full");
    }
