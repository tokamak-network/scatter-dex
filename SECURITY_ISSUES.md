...
| 44 | zk-relayer · 프라이버시 | 공개 `trade-offers` 엔드포인트 트레이더 식별자 노출 (감사 A-1, MEDIUM) | ✅ | PR #1083 (`aa40c06e`) — 공개 `GET /api/p2p/trade-offers` 응답에서 트레이더 pubkey 등 식별 컬럼을 admin과 동일 기준으로 제거(프라이버시 invariant 일관성). 2026-06-25 전체모듈 감사 A-1. 상세: `보안_전체모듈_감사_2026-06-25.md` §A-1. (2026-06-25) |
