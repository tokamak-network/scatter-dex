...
| 34 | apps | CSP `'unsafe-eval'` 처리 | ✅ | **2026-05-13 클로즈.** 배포 대상이 되는 타겟에서만 처리: PR #725로 hub + pay `firebase.json`에 strict CSP 적용 (`script-src 'self' 'unsafe-inline'`, eval 없음, `worker-src 'self' blob:`, `connect-src 'self' https: wss:`, Google Fonts allow-list, `frame-ancestors 'none'`, nosniff, Referrer-Policy). 다른 타겟 처리 방침: (a) **`frontend/`** — 배포 안 함 (참조 구현 only), `next.config.ts`의 `unsafe-eval`은 dev-only 영향이라 그대로 둠. 만약 향후 배포되면 그 시점에 worker 마이그가 이미 완료되어 있어 5분 작업으로 닫을 수 있음 (확인: snarkjs는 `*-worker.ts` 4개에서만 호출, 메인스레드 호출 0건). (b) **docs / pro / relayer** — 미오픈이라 보류. 재오픈 시 hub(static) 또는 pay(ZK) 패턴 복사. (2026-05-13) |
