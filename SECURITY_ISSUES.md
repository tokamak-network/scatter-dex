...
| 36 | shared-OB | 미검증(verified=0) settlement 행이 leaderboard/volume에 합산 | ⚠️ | PR #693 (커밋 `9ca75a6d`) — `volumeByTokenVerified` / `pairsVerified` 노출 + frontend `RelayerTradeStats`가 verified-only를 디폴트로 렌더. 그러나 Phase 2.5b verify job(`verified=1` 백필)은 여전히 미구현이라 verified-only 집계가 모든 환경에서 0. 다음: PR로 verify job 출시 — relayer가 push한 settlement의 tx_hash가 실제 `PrivateSettledAuth` 이벤트에 매칭되는지 확인 후 `verified=1` 마킹. (2026-05-12) |
...
| 39 | shared-OB · test | shared-OB vitest 베이스라인 35건 실패 + body-hash / SSRF / verify-job 테스트 미커버 | ⬜ | (a) `main` 베이스라인에서 `shared-orderbook` vitest 35건 실패 — 이전 PR 누적으로 깨졌고 PR #689/#693 이전에도 동일 (확인됨, PR #689는 그 중 11건을 회복시킴). 회복 PR 필요. (b) PR #693의 body-hash 바인딩 + `REQUIRE_BODY_HASH=1` fail-closed 모드, PR #689의 SSRF guard private-IP / DNS rebind 거부 동작은 unit-test 미커버. 베이스라인 회복 후 한 PR에서 같이 처리 권장. (2026-05-12) |
| 40 | zk-relayer · test | p2p 라우트 body-hash / SSRF guard 테스트 미커버 | ⬜ | `zk-relayer/src/routes/p2p.ts`의 `verifyRelayerAuth` body-bound message accept / legacy fallback / `REQUIRE_BODY_HASH=1` reject 경로 unit test 없음. `shared-orderbook-client.ts`의 outbound `assertSafeOutboundUrl` 가드도 마찬가지. 항목 #39와 같은 PR로 처리. (2026-05-12) |
