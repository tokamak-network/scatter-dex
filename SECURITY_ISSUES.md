...
| R-14 | relayer | 부하 테스트 (k6/artillery) | 🔧 | **이번 PR** (`test/loadtest-r14-shared-ob` branch) — k6 기반 shared-orderbook 시나리오 4개: smoke / orders / leaderboard / settlements. 프로필 smoke(10 RPS, 30s) / load(100 RPS, 1.5min) / stress(1000 RPS, 3.5min). 관측 대상: 서명-검증 미들웨어 처리량 + sqlite 집계 read p95. CI는 `workflow_dispatch` 수동 트리거 (`.github/workflows/load-test.yml`) — 러너 CPU 변동성 때문에 PR gate 안 됨. zk-relayer / on-chain settle 부하는 별도 트랙 (소요 환경 다름). |
