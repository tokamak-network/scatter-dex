...
| 34 | frontend / apps | CSP `'unsafe-eval'` 처리 | ⚠️ | `frontend/` (참조 구현, 비활성 데모): 여전히 main-thread snarkjs를 호출하므로 `unsafe-eval` 잔존. 미오픈 상태라 후순위. **이번 PR (2026-05-13)** — 곧 오픈 예정 타겟 (hub, pay) 에 strict CSP 적용. `firebase.json` headers 블록에 `script-src 'self' 'unsafe-inline'` (eval 없음!) + `worker-src 'self' blob:` (pay는 snarkjs를 WebWorker로 격리) + `connect-src 'self' https: wss:` (RPC + relayer + WebSocket) + `frame-ancestors 'none'` (clickjacking 방지) + `X-Content-Type-Options: nosniff` + `Referrer-Policy: strict-origin-when-cross-origin`. hub는 정적 CSP (ZK/worker 없음). 다른 타겟 (docs/pro/relayer) 은 미오픈이라 손대지 않음 — 재오픈 시 hub/pay에서 복사. (2026-05-13) |
