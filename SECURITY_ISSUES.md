...
| 46 | shared-OB · DoS | `POST /api/settlements` settlement 행 무제한 증가 → 메모리/디스크 DoS (감사 A-3, MEDIUM) | ✅ | PR #1085 (`60c64803`, K1) — `insertSettlement`에 상한 없고 `getRelayerSettlementStats`가 전 행을 힙에 적재, 게이트 없는 쓰기로 가짜 행 누적 가능했음. 수정: `shared-orderbook/src/core/db.ts`에 전역 행 상한 `MAX_SETTLEMENTS`(env, 기본 200000) + 서버 스탬프 `created_at` 기준 오래된 행 주기적 prune(인터벌 tick try/catch 가드) + maker/taker nullifier 파싱 경계 소문자화(인덱스 TEXT 컬럼 중복 표현 방지) + `pruneSettlements` `maxRows<1` 거부. 행 상한은 글로벌 disk-DoS 백스톱(per-chain 격리는 후속). 테스트 `db.test.ts`/`settlements.test.ts` 추가. 2026-06-25 감사 A-3. (2026-06-25) |
| 47 | contracts · 컴플라이언스 | `RelayerRegistry._isOperational` 이 KYC 승인을 운영 중 재검증 안 함 (감사 B-1, MEDIUM) | ✅ 코드 / ⏳ 온체인 | PR #1087 (`6b8a5453`, K2) — register 시점만 KYC 체크하고 운영모드(`_isOperational`)에서 재검증이 없어, 승인 철회 후에도 정산 계속 가능했음("승인 유지" 정책 우회). 수정: `operationalKycRequired` 옵트인 플래그(fail-closed) 추가 — 켜지면 운영 게이트가 IdentityRegistry 승인을 재확인. 봇리뷰 반영: `NotAContract` 가드. **온체인 반영 대기**: Sepolia 프록시 업그레이드(`UpgradeRelayerRegistryKycRecheck.s.sol`)는 사용자 승인 HOLD — 코드만 머지, 온체인 미적용. 2026-06-25 감사 B-1. (2026-06-25) |
| 48 | contracts · 설정의존 | `_pullBond` fee-on-transfer/rebasing 토큰 시 bond 회계 불일치 (감사 B-2, LOW) | ✅ 코드 / ⏳ 온체인 | PR #1087 (`6b8a5453`, K2, B-1과 묶음) — `_pullBond`가 transfer 전후 balance delta를 측정하지 않아 fee-on-transfer 토큰에서 기록 bond > 실수령 bond 가능했음. 수정: balance-delta 측정으로 실수령액 기록 + underflow revert 가드(봇리뷰 반영). B-1과 동일 프록시 업그레이드라 온체인 반영은 동일 HOLD. `MockFeeOnTransferToken.sol` 테스트. 2026-06-25 감사 B-2. (2026-06-25) |
| 49 | zk-relayer · DoS | `RemoteOrderStore` 무제한 증가 (`MAX_ORDERBOOK_SIZE` 정의만 되고 미사용) + `/api/p2p` rate limit 부재 (감사 A-2, MEDIUM) | ✅ | PR #1089 (`e2d9bc78`, K0) — permissionless `POST /api/p2p/orders` 플러딩으로 `byId`/`byRelayer`/정렬 pair 리스트가 무제한 증가(O(N²)) 가능했음. 수정: `zk-relayer/src/core/remote-orderbook.ts` `add()`에 `MAX_ORDERBOOK_SIZE` 상한 실제 적용 + `/api/p2p`에 rate limit(`zk-relayer/src/index.ts`). 봇리뷰 반영: strict-integer cap 파싱, fake-timer try/finally, trust-proxy 주석. 테스트 `remote-orderbook.test.ts` 보강. 2026-06-25 감사 A-2. (2026-06-25) |
| 50 | shared-OB · 모니터링 | 검증불가 settlement 행이 `verify-stats` 알림을 영구 오염 (감사 A-5, LOW-MED) | ✅ | PR #1088 (`a4d08d04`, K1) — 가짜 행(`blockNumber=0`) 1건이 `GET /api/admin/verify-stats`의 `hasUnverifiedRows`/`oldestUnverifiedBlock`를 영구 트리거해 실제 검증 실패를 가렸음. 수정: `shared-orderbook/src/core/verifier.ts`/`db.ts`에 "pending" vs "verify-impossible" 상태 구분 + 실패 행 격리/pruning + 넓은 range에서 verifier stall 수정 + 인덱스 마이그레이션 순서 보정(봇리뷰). 테스트 `verifier.test.ts`/`admin.test.ts`/`db.test.ts` 추가. 2026-06-25 감사 A-5. (2026-06-25) |
| 51 | zk-relayer · 그리핑 | P2P trade-offer taker proof 미검증 + `/api/p2p` rate limit 부재 (감사 A-4, LOW-MED) | ✅ | PR #1093 (`4b0d5459`, K0) — `handleTradeOffer`가 taker proof를 검증하지 않고 token/price/expiry/fee만 보고 `submitAuthSettle` 호출 → permissionless peer가 junk proof 플러딩으로 글로벌 `txMutex` 직렬화 + `eth_estimateGas` RPC 쿼터 소모 가능했음. 수정: mutex/estimate 전에 `verifyAuthorizeProof(takerOrder)` 호출 + `/api/p2p` rate limit. 봇리뷰 반영: verified array를 named public signal에 바인딩(array↔named soundness 갭) + malformed taker 거부. 테스트 `cross-relayer-tradeoffer-verify.test.ts`. 2026-06-25 감사 A-4. (2026-06-25) |
| 52 | shared-OB · 그리핑 | `POST /api/kyc/submit` 지갑 소유 증명 없음 (피해자 KYC 덮어쓰기 + 디스크 채우기) (감사 A-6, LOW) | ✅ | PR #1091 (`71c8c02e`, K1) — `wallet`이 서명 바인딩 없는 평문 필드라, 공개 `GET /api/kyc/status?wallet=`로 피해자 pending 확인 후 동일 wallet 제출 → 피해자 email/video/idDoc 덮어쓰기 + 큐 위치 리셋 + 임의 wallet마다 디스크 폴더(최대 25MB×2) 생성 가능했음. 수정: 업로드 전 헤더 기반 지갑 소유 증명(EIP-191/SIWE 서명 검증) + operators 서명 경로. `shared-orderbook/src/routes/kyc.ts` + `config.ts`. 테스트 `kyc.test.ts` 보강. 2026-06-25 감사 A-6. (2026-06-25) |
| 53 | contracts · 죽은코드 | `PrivateSettlement.scatterDirect` claimsRoot 미바인딩 함수 제거 (감사 scatterDirect, MEDIUM) | ✅ | PR #1094 (`58faeb21`, K2) — withdraw proof가 claimsRoot를 바인딩하지 않아 relayer가 임의 claimsRoot 등록 가능했던 죽은코드 경로. 프로덕션은 proof-bound `scatterDirectAuth`로 대체됨. 수정: `scatterDirect` 함수 + `ScatterDirectParams` 구조체 + `ScatterDirect` 이벤트 + 고아 `onlyRelayer`/`_routeFeeLocal` 제거, 테스트는 `scatterDirectAuth`로 retarget(커버리지 보존). forge 601 통과. 함수 제거는 PrivateSettlement 재배포 시점 온체인 반영. 2026-06-25 감사 scatterDirect. (2026-06-25) |
