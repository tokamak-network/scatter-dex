...
| 34 | frontend / apps | CSP `'unsafe-eval'` 처리 | ✅ | **2026-05-13 양쪽 모두 클로즈.** apps 트랙: PR #725에서 `firebase.json`에 hub + pay strict CSP 적용 (`script-src 'self' 'unsafe-inline'`, eval 없음, `worker-src 'self' blob:`, Google Fonts allow-list, `frame-ancestors 'none'`, nosniff, Referrer-Policy). frontend/ 트랙: 본 정리 PR — snarkjs는 이미 `*-worker.ts` 4개 파일에서만 호출 (`warmProverAssets`/`zkey-cache.ts:189`의 `import("snarkjs")`가 grep상 worker context 외 호출 0건) → `frontend/next.config.ts:37`의 `'unsafe-eval'` 은 vestigial. 제거 → frontend도 main-thread `eval` 차단. 향후 main-thread snarkjs 호출 사이트가 들어오면 CSP violation으로 즉시 노출됨. 다른 미오픈 타겟 (docs/pro/relayer)은 재오픈 시점에 hub/pay 패턴 복사. (2026-05-13) |
