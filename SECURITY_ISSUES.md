...
| 35 | shared-OB | 서명 메시지에 body hash 미포함 (replay-modify within 5min window) | ⬜ | `shared-orderbook/src/middleware/auth.ts:45` 의 서명 포맷이 method/path/url만 커버, body 미포함. TLS 종단 또는 in-path 공격자가 5분 안에 합법 요청을 가로채 body만 바꿔 재전송 가능. `tx_hash` PRIMARY KEY + `INSERT OR IGNORE` 가 대부분 막지만 race-condition으로 합법 write를 이긴 경우 fee/amount 필드 조작 가능. 수정: 서명 메시지에 `:{sha256(canonicalJson(body))}` 추가 + SDK signRelayerHeaders 갱신. PR-B 처리 예정 (2026-05-12). |
| 36 | shared-OB | 미검증(verified=0) settlement 행이 leaderboard/volume에 합산 | ⬜ | `shared-orderbook/src/routes/settlements.ts:50-72` + `db.ts:549-659`. 등록된 relayer가 임의 tx_hash로 자기 자신을 makerRelayer로 지정한 페이크 settlement push → 리더보드/네트워크 볼륨 인플레이션. Phase 2.5b verify job 미구현. 수정: (a) verify job 출시 + `verified=1` 게이트, 또는 (b) `volumeByTokenVerified`/`countVerified` 변종 + 프론트 디폴트 전환. PR-B 처리 예정 (2026-05-12). |
