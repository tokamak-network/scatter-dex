...
| Gas per trade | ~150K | ~100K | ~500K+ | ~300K+ | **~410K** |
...
We implement Scatter Settlement as a Solidity smart contract using the Foundry framework, deployed on Ethereum L2 (Arbitrum Sepolia). The implementation consists of:

- `ScatterSettlement.sol`: Core contract (~300 lines)
- `IdentityGate.sol`: zk-X509 authentication wrapper (~50 lines)
...
|-----------|--------------------|--------------|---------|
| Deposit | ~60K gas | ~1M gas (Merkle insert) | ~500K gas |
| Settle (2 parties, 3+1 claims) | ~200K gas | N/A | N/A |
| Claim (per recipient) | ~50K gas | ~300K gas (ZK verify) | ~300K gas |
| **Total (1 trade, 3 recipients)** | **~410K gas** | **~2.2M gas** | **~1.7M gas** |
| Privacy approach | Hash-lock + time-lock | ZK Merkle proof | zk-SNARK |

*Note: Gas costs are estimates pending implementation. Final numbers will be measured on L2 testnet.*
...
*To be completed after Phase 1 implementation. Will include:*
- *Foundry gas reports for each function*
- *Comparison methodology against Tornado Cash and Railgun contracts*
- *L1 vs L2 cost analysis*
